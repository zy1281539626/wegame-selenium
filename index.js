...
const isExist = async(ele) => {
  var bool = null
  await driver.findElement(By.id(ele)).then(function(result){
    // 获取成功时的处理
    console.log("元素找到")
    bool = true
...
const aa = async () => {
  ...
  setInterval(async () => {
    ...
    console.log('check')
    let a = await isExist("tcaptcha_transform");
    if (a === true) {
      console.log('true...')
      /**
       * 进入验证码的iframe里
...
const bb = async () => {
  ...
  while (Math.abs(offset - current) > 5) {
    ...
    current += distance
    await actions.move({ origin: slide, x: distance, duration: 20 })
  ...
  isMove = false
  setTimeout(() => { 
    driver.findElement(By.id("captcha_close")).click()
  }, 500)
